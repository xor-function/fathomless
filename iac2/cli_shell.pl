...
sub isalive{
        ...
        my $info = '[+] ALIVE : ' . $sysname . '|' . $uuid . ' |[Last Checkin]: '. $lastmod;
        return $info;
...
sub isdead{
        ...
        my $info = '[-] STALE : ' . $sysname . '|' . $uuid . ' |[Last Checkin]: '. $lastmod;
        return $info;
}
   		...
   		if ((!($cont =~ /-command/ )) && (!($cont =~ /-stdout/))) {
			...
			my $info;
        		...
        		if ( $cyday eq $yday )
        		{
                		...
                		my $pdif = abs($dif);
                		if ( $pdif >= '0' && $pdif <= '30' ) {
                                        $info = isalive($sysname, $uuid, $lastmod, $currenttime);
					push @systemlist, $info;
                		} else { 
					$info = isdead($sysname, $uuid, $lastmod, $currenttime); 
					push @systemlist, $info;
				}
        		}
       			 else {
